...
网页把你的脸压成果蝇复眼看到的 817 个六边形亮度读数，送进一个按 MaleCNS 连接组视觉通路简化出来的脉冲神经网络，给出一个**果蝇眼中的颜值分**（40–99）和段位称号，外加果蝇的反应：它想**靠近**你、看呆了，还是想**逃跑**。

打分的是果蝇：算分前会把明暗和对比度拉平，肤色和打光不影响分数；剩下的差别很小，主要是 200 ms 里的放电噪声，所以同一张照片永远同分，换一张可能就变了。分数曲线故意偏宽容，平均约 74 分，最低 40 分。没有排行榜，分数也不出设备。
...
摄像头 / 上传照片
  ...
  → 颜值分 40–99（归一化后再跑一轮）+ 反应：APPROACH · HESITATE · ESCAPE
```
...
| 颜值分 | `src/lib/fly-score.mjs` — 小眼亮度先归一化（均值 0.5、标准差 0.2），同一个种子再跑一轮，逃跑/靠近之差按真实人脸的分布做 logistic 校准到 40–99（偏宽容，均值约 74），分五档称号。不归一化的话分数几乎只反映亮度，也就是肤色和打光 |
